...
angular.module('MetronicApp').controller('UserSecurityEditController', function ($scope, $element, $http, title, items, close) {

    $scope.title = title;
    $scope.newUserSecurity = items.newUserSecurity;
    $scope.UserSecurity = items.UserSecurity;
    ...
    $scope.ParseTreeArr = function () {
        ...
        for (var i = 0; i < $scope.TreeArr.length; i++) {
            ...
            var Module = $scope.newUserSecurity.Modules.filter(function (el) { return el.sModuleHTMLID == moduleID; })
    ...
    $scope.PreviousRoleChange = function () {
        if ($scope.newUserSecurity.prevRole != undefined && $scope.newUserSecurity.prevRole != "") {
            $scope.newUserSecurity = $scope.UserSecurity[$scope.UserSecurity.findIndex(x=> x.sRoleID == $scope.newUserSecurity.prevRole)]
            $scope.Security_Tree_rowListJson = [];
